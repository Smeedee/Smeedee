    ...
    public class LatestCommitsController : ControllerBase<LatestCommitsViewModel>
    {
        ...
        private void LoadColorSetting(SettingsEntry setting)
        {
            uiInvoker.Invoke(() =>
            {
                ...
                for (int i = 0; i < config.Count()-1; i += 2)
                {
        ...
        private void SaveSettings()
        {
            ...
            {
                if (string.IsNullOrEmpty(keywordColor.Keyword))
                    continue;
        ...
        {
            var pair = new KeywordColorPairViewModel { Keyword = "word" + ViewModel.KeywordList.Count(), ColorName = "grey" };

...
{
    [Export(typeof(Slide))]
    public class CommitStatisticsModule : Slide
    {
        private CommitStatisticsController _controller;
        private const long REFRESH_INTERVAL = 1000 * 30; // ms * sec * min


        public CommitStatisticsModule()
        {
            Title = "Commit Statistics";

            var viewModel = new BindableViewModel<CommitStatisticsForDate>();
            View = new CommitStatisticsView();
            ...
            _controller = new CommitStatisticsController(viewModel, changesetRepository, asyncClient, timer, uiInvoker,configRepostory, configPersister, logger);
       }
